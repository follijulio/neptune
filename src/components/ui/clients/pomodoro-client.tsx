...
const PhaseButton = ({
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 rounded-2xl border px-3.5 py-1.5 text-[10px] font-bold tracking-wider uppercase transition-all duration-200 ${
        isSelected
          ? config.pill
...
const StatusIndicator = ({ currentConfig, isActive }: StatusIndicatorProps) => {
  ...
  return (
    ...
    >
      <span
        className={`h-1.5 w-1.5 rounded-2xl ${currentConfig.activeDot} ${
...
const ControlButtons = ({
  ...
  <div className="flex w-full items-center gap-3">
    <Button
      onClick={onToggle}
      className={`h-14 flex-1 rounded-xl text-sm font-black tracking-wide transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${
        isActive
