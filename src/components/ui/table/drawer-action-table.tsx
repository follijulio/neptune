...
const CourseDrawerAction: React.FC<CourseDrawerActionProps> = ({ course }) => {
  ...
  return (
    <Drawer direction="right" open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DrawerContent className="flex h-full flex-col border-l border-[#1A1A1A] bg-[#0A0A0A] px-4 text-white sm:max-w-2xl sm:px-6">
        <DrawerHeader className="border-b border-[#1A1A1A] px-0 pt-6 pb-4 sm:pt-8">
          <div className="flex items-center justify-between">
            <div className="pr-2">
              <DrawerTitle className="line-clamp-2 text-xl font-bold sm:text-2xl">
                {course.subject_name}
              </DrawerTitle>
              <DrawerDescription className="mt-1 text-xs text-zinc-500 sm:text-sm">
            ...
            {view !== "list" && (
              ...
              >
                <LuArrowLeft className="mr-1 h-4 w-4 sm:mr-2" />
        ...
        <div className="custom-scrollbar flex-1 overflow-y-auto py-4 sm:py-6">
          {isLoading ? (
            ...
            <div className="space-y-4 sm:space-y-6">
              <section className="rounded-xl border border-zinc-900 bg-[#121212]/50 p-3 sm:p-4">
                <div className="flex items-center justify-between">
                  <button
                    onClick={() => toggleSection("exams")}
                    className="flex items-center gap-2 text-xs font-bold tracking-wider text-zinc-400 uppercase transition-colors hover:text-white sm:text-sm"
                  >
                    {openSections.exams ? (
                      <LuChevronDown className="text-base sm:text-lg" />
                    ) : (
                      <LuChevronRight className="text-base sm:text-lg" />
                    )}
                    <LuCalendar className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Provas
                  </button>
                  <Button
                    size="sm"
                    onClick={() => setView("add-exam")}
                    className="h-7 bg-[#007AFF]/10 px-2 text-[10px] text-[#007AFF] hover:bg-[#007AFF]/20 sm:px-3 sm:text-xs"
                  >
                    + Adicionar
                  </Button>
                </div>
                ...
                {openSections.exams && (
                  <div className="animate-in fade-in-0 slide-in-from-top-2 mt-3 duration-200 sm:mt-4">
                    {sortedExams.length === 0 ? (
                      <p className="text-xs text-zinc-600 italic sm:text-sm">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        {sortedExams.map((exam) => (
                          <div
                            key={exam.id}
                            className="flex flex-col gap-1 rounded-xl border border-zinc-800/50 bg-zinc-900/50 p-2.5 sm:p-3"
                          >
                            <span className="text-xs font-semibold sm:text-sm">
                              {exam.title}
                            </span>
                            <span className="text-[10px] font-medium text-[#007AFF] sm:text-xs">
              ...
              <section className="rounded-xl border border-zinc-900 bg-[#121212]/50 p-3 sm:p-4">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    <LuPaperclip className="h-3.5 w-3.5 sm:h-4 sm:w-4" />{" "}
                    Materiais
                  </button>
                  ...
                  <UploadButton
                    ...
                    appearance={{
                      button:
                        "h-7 px-2 sm:px-3 bg-[#007AFF]/10 text-[10px] sm:text-xs text-[#007AFF] hover:bg-[#007AFF]/20 font-semibold rounded-md border-none w-auto m-0",
                  ...
                  <div className="animate-in fade-in-0 slide-in-from-top-2 mt-3 duration-200 sm:mt-4">
                    {materials.length === 0 && !isUploadingPDF ? (
                      ...
                      <div className="space-y-2">
                        {isUploadingPDF && (
                          <div className="flex animate-pulse items-center gap-2 rounded-xl border border-zinc-800/50 bg-zinc-900/30 p-2.5 sm:gap-3 sm:p-3">
                            <LuLoader className="h-3.5 w-3.5 animate-spin text-[#007AFF] sm:h-4 sm:w-4" />
                        ...
                        {materials.map((mat) => (
                          <a
                            ...
                            className="group flex items-center justify-between rounded-xl border border-zinc-800/50 bg-zinc-900/50 p-2.5 transition-colors hover:border-[#007AFF]/50 hover:bg-zinc-800/80 sm:p-3"
                          >
                            <div className="flex flex-col gap-0.5 overflow-hidden pr-3 sm:gap-1 sm:pr-4">
                              <span className="truncate text-xs font-semibold text-zinc-200 group-hover:text-white sm:text-sm">
                                {mat.name}
                              </span>
                              <span className="text-[9px] font-medium text-zinc-500 sm:text-[10px]">
              ...
              <section className="rounded-xl border border-zinc-900 bg-[#121212]/50 p-3 sm:p-4">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    <LuFileText className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Caderno
                  </button>
                  <Button
                    size="sm"
                    onClick={() => setView("add-note")}
                    className="h-7 bg-emerald-500/10 px-2 text-[10px] text-emerald-500 hover:bg-emerald-500/20 sm:px-3 sm:text-xs"
                  >
                    + Nova Nota
                  </Button>
                </div>
                    ...
                    {notes.length === 0 ? (
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        {notes.map((note) => (
                          <div
                            key={note.id}
                            className="group rounded-xl border border-zinc-800/50 bg-zinc-900/50 p-3 transition-colors hover:border-zinc-700 sm:p-4"
                          >
                            <div className="mb-2 flex items-center justify-between">
                              <h4 className="text-xs font-semibold text-zinc-200 sm:text-sm">
                                {note.title}
                              </h4>
                              <button
                                onClick={() => {
                                ...
                                className="text-zinc-500 opacity-0 transition-all group-hover:opacity-100 hover:text-[#007AFF] focus:opacity-100"
                                title="Editar anotação"
                              >
                                <LuPencil className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                              </button>
                            </div>
                            <div className="prose prose-sm prose-invert prose-p:leading-relaxed prose-pre:bg-zinc-950 prose-pre:border prose-pre:border-zinc-800 line-clamp-4 max-w-none text-[11px] text-zinc-400 sm:text-sm">
            ...
            <form
              ...
              className="flex h-full flex-col space-y-3 sm:space-y-4"
            >
              ...
              <Input
                ...
                required
                className="h-10 border-zinc-800 bg-zinc-900 text-sm sm:h-12"
              />
              <textarea
                ...
                required
                className="w-full flex-1 resize-none rounded-md border border-zinc-800 bg-zinc-900 p-3 text-xs text-white focus-visible:ring-1 focus-visible:ring-[#007AFF] focus-visible:outline-none sm:text-sm"
              />
              <Button
                type="submit"
                disabled={isSaving}
                className="h-10 w-full bg-emerald-600 text-sm font-bold hover:bg-emerald-700 sm:h-12 sm:text-base"
              >
            ...
            <form onSubmit={handleAddExam} className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-semibold text-zinc-400 sm:text-xs">
                  Título / Descrição
                </label>
                <Input
                ...
                <div className="flex w-full flex-col items-start gap-2 sm:flex-row sm:items-center">
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={"outline"}
                        className={cn(
                          "h-10 w-full justify-start truncate border-zinc-800 bg-zinc-900 text-left text-xs font-normal sm:h-12 sm:flex-1 sm:text-sm",
                  ...
                  <div className="flex w-full shrink-0 items-center justify-between gap-1 sm:w-auto sm:justify-start">
                    <Select value={examHour} onValueChange={setExamHour}>
                      <SelectTrigger className="h-10 w-[calc(50%-0.5rem)] border-zinc-800 bg-zinc-900 text-xs focus:ring-[#007AFF] sm:h-12 sm:w-16.25 sm:text-sm">
                        ...
                        {Array.from({ length: 24 }).map((_, i) => {
                          ...
                          return (
                            <SelectItem
                              key={val}
                              value={val}
                              className="text-xs focus:bg-zinc-800 focus:text-white sm:text-sm"
                            >
                              {val}
                            </SelectItem>
                          );
              ...
              <div className="flex w-full flex-wrap justify-between gap-2 px-1 pt-2 sm:flex-nowrap sm:gap-3 sm:px-3">
                {NOTE_COLORS.map((c) => (
                  <button
                    key={c.value}
                    type="button"
              ...
              <Button
                type="submit"
                disabled={isSaving}
                className="mt-4 h-10 w-full bg-[#007AFF] text-sm font-bold hover:bg-[#005bb5] sm:h-12 sm:text-base"
              >
